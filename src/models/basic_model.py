...
class BasicModel(object):
    ...
    def build_graph(self, graph):
        ...
        with graph.as_default():
            ...
            with tf.name_scope('rnn'):
                ...
                ##############################################
                # To be able to feed the final state/state of the LSTM back into the network
                # we have to create a placeholder with the same size as the final state
                ...
                # into a tuple. Then can be fed into the net as an initial state
                ...
                for i in range(self.n_layers):
                    # We use several layers of LSTM cells with peephole connections and dropout
                    ...
                    cell = tf.contrib.rnn.LSTMCell(
                        self.n_hidden, use_peepholes=True, initializer=tf.contrib.layers.xavier_initializer())
    ...
    def train(self, save_every=20):
        ...
        with self.graph.as_default():
            ...
            # We run the training as long as possible. Alternatively, the training can run
            # for a specified number of steps
            try:
            # for epoch_id in range(0, self.max_iter):
                while not self.coord.should_stop():

                # If the inputs should be clipped
                # invoke the get_padded batch method to receive
                # inputs, lengths and labels.
                ...
                # _labels = clip_batch(_labels, _lengths)
                # tf.logging.debug("Input sequence: {}".format(self.sess.run(tf.argmax(_inputs, axis=2))[0]))
                # tf.logging.debug("Target sequence: {}".format(_labels[0]))


                # Run the training step, either with clipped inputs and labels
                # (if so, include them in the feed_dict) or the unclipped ones.
                # Decide whether the final state of the net is fed back into the net
                # if so, include it in the feed_dict
                    ...
                    # Retrieve the predicted notes
                    pred = _predictions_flat.reshape(128, -1)[0]
                    epoch_id += 1
                    ...
                    if save_every > 0 and epoch_id % save_every == 0:
                        ...
                        np.savetxt(os.path.join(p, 'final_state_00.csv'), _current_state[0][0])
                        np.savetxt(os.path.join(p, 'final_state_01.csv'), _current_state[0][0])
                        np.savetxt(os.path.join(p, 'final_state_10.csv'), _current_state[0][0])
                        np.savetxt(os.path.join(p, 'final_state_11.csv'), _current_state[0][0])
    ...
    def test(self):
        ...
        # Load the final LSTM state from the checkpoints folder
        p = os.path.join(HOME, 'deepmusic/models/checkpoints/')
        feed00 = np.loadtxt(os.path.join(p,'final_state_00.csv'))
        feed01 = np.loadtxt(os.path.join(p,'final_state_01.csv'))
        feed10 = np.loadtxt(os.path.join(p,'final_state_10.csv'))
        feed11 = np.loadtxt(os.path.join(p,'final_state_11.csv'))

        t1 = tuple((feed00, feed01))
        t2 = tuple((feed10, feed11))
        _current_state = np.array([t1, t2])
    ...
    def generate(self):
        ...
        with self.sess as sess:
            ...
            # way we can generate several different melodies simultaneously
            ...
            # Load the final state of the LSTM and transform it into
            # tuples such that it can be fed into the network
            p = os.path.join(HOME, 'deepmusic/models/checkpoints/')
            feed00 = np.loadtxt(os.path.join(p, 'final_state_00.csv'))
            feed01 = np.loadtxt(os.path.join(p, 'final_state_01.csv'))
            feed10 = np.loadtxt(os.path.join(p, 'final_state_10.csv'))
            ...
            for i in range(90):
                _logits_flat, _current_state, _predictions_flat = self.sess.run(
                    [self.logits_flat, self.final_state, self.predictions_flat],
                    feed_dict={
    ...
    def init(self):
        ...
        if checkpoint is None:
            ...
            # Even when restoring the model we have to initialize
            # the local variables. Otherwise an error is thrown
